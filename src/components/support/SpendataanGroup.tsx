...
interface Icdt {
  judul: string;
  field: string[];
  sendValue: (data: IcdtItem[]) => void;
  initialData?: IcdtItem[];
}

export function SpendataanGroup({ judul, field, sendValue, initialData }: Icdt) {
  const [dlist, setDlist] = useState<IcdtItem[]>(initialData?.filter(obj => Object.values(obj)[0] !== '') || []);
  const [inputValue, setInputValue] = useState('');
  const [editingNama, setEditingNama] = useState<string | null>(null);
